...
* **Production Queue** — the set of `PizzaTask`s with `status = Pending`. Not a separately-maintained projection — a direct query over `PizzaTask` (`08_kitchen_domain_model.md` §4), listed here because `02_discover_process_level.md` §1.3.1 names it as a read model a Chef consults.
