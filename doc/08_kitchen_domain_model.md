...
* **`FinishPizza` splits the same way `AcceptOrder` does.** `PizzaTask`'s own completion (`PizzaPrepared`) stays internal — it already feeds two internal consumers, the Order Progress read model above and the Busy Chefs read model (`08_kitchen_read_models.md`, marking the chef free again). A third fact, "which chef, right now," is needed externally by Resource Management's `Chef` to know when a `Terminating` chef's in-hand pizza is done (`02_discover_process_level.md` §5) — published as its own event, `ChefFinishedPizza` (`chefId`), on every `FinishPizza`, unconditionally (Kitchen doesn't check or care whether the chef is `Terminating`; `08_resource_management_aggregates.md` §4 decides that on its own side). `02` §5's original policy read "Whenever `PizzaPrepared`... and the chef... is `Terminating`" — that phrasing predates this split; the actual cross-context signal is `ChefFinishedPizza`, not `PizzaPrepared` (`design_notes/dn_0001.md`).
