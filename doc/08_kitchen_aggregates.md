...
* `correlationId`: copied from the inbound `OrderSentToKitchen` envelope at creation and never changed. Not domain data — envelope metadata (`09_integration_contracts.md` §2) kept on the aggregate so `OrderReadyForPickup` can carry it forward later, since that event is raised as the tail end of a chain of purely internal events (`PizzaPrepared` → Order Progress → `OrderReadinessCheck` → `MarkOrderReady`), with no inbound integration event around at that point to copy it from directly.
...
2. **`AcceptOrder` is idempotent by `kitchenOrderId`: if a `KitchenOrder` with this ID already exists, the command no-ops instead of creating a duplicate.** `OrderSentToKitchen` arrives over an at-least-once broker (`09_architecture.md` §5–§6), so redelivery must be assumed; since `kitchenOrderId` correlates 1:1 and deterministically with Guest Service's `orderId` (above), the existence check is cheap and exact. This is the same discipline DN-2 (`design_notes/dn_0002.md`) already applies to accumulator fields, extended one step earlier to aggregate creation itself: without it, a redelivered `OrderSentToKitchen` would spawn a second `KitchenOrder` and duplicate every `PizzaTask` for the same order.
3. **`MarkOrderReady` only fires once the Order Progress read model's completed set reaches `totalPizzaCount` in size**, and only ever auto-triggered by the `PizzaPrepared` that makes it reach that size (`08_kitchen_domain_model.md` §3) — never called directly. Checked by `OrderReadinessCheck` (`08_kitchen_domain_services.md`), not by a field this aggregate maintains itself.
4. **`totalPizzaCount` is fixed at creation.** No command changes `lines` after `AcceptOrder` — matches `02_discover_big_picture.md` §5 ("No order cancellation once sent to the kitchen"). This is exactly why it's safe to keep on the aggregate directly, unlike the ready-tracking above: it's written once, never mutated.
