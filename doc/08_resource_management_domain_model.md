...
Owns: `capacity`, `status` (`Free`/`Occupied` — mirrored from Guest Service, not decided here), `assignedWaiterId` (nullable). Driven by commands: `AddTable`, `ChangeTableCapacity`, `RemoveTable`, `AssignTableToWaiter`, `UnassignTableFromWaiter` (`02_discover_process_level.md` §2), plus reactions to Guest Service's own `TableAssigned`/`TableReleased`.

### MenuItem

**Identity:** `menuItemId`.

Owns: `name`, `ingredients`, `recipe`, `price`. Driven by commands: `AddMenuItem`, `UpdateMenuItem`, `RemoveMenuItem` (`02_discover_process_level.md` §3). No relationships to any other aggregate in this context — a self-contained catalog.

### Waiter

**Identity:** `waiterId`.

Owns: `status` (`Active` → `Terminating` → `Terminated`). Driven by commands: `HireWaiter`, `StartWaiterTermination`, `FinalizeWaiterTermination` (auto) (`02_discover_process_level.md` §4). Does **not** own which tables it serves — that's `Table.assignedWaiterId`, referenced by ID only (`03_decompose_subdomains.md` §5 Decisions).

### Chef

**Identity:** `chefId`.

Owns: `status` (`Active` → `Terminating` → `Terminated`). Driven by commands: `HireChef`, `StartChefTermination`, `FinalizeChefTermination` (auto) (`02_discover_process_level.md` §5).
...
* **Table Directory** — `tableId → { capacity, status, assignedWaiterId }`, fed by every `Table` event (`08_resource_management_read_models.md`). Feeds: `RemoveTable`'s "not the last table" guard (count of entries); `Waiter`'s `FinalizeWaiterTermination` check ("no `Occupied` table left pointing at this waiter" — filter by `assignedWaiterId`, `08_resource_management_aggregates.md` §3); and is also what this context's exposed **Available Tables** / **Waiter Workload** read models (`02_discover_process_level.md` §2) are computed from.
