...
* **Whenever `OrderPlaced`** → the order's lines are recorded in the **Bill Summary** read model, keyed by `orderId` (`08_guest_service_read_models.md`), matching `02_discover_process_level.md` §1.2's policy verbatim ("the order's lines... are added to this bill and its total is recalculated"). Keyed, not accumulated: a redelivered `OrderPlaced` for the same order overwrites the same entry rather than adding a second one, so the derived total can't be inflated by duplicate delivery — see `README.md` Design Notes DN-2. `Bill` itself holds no total (`08_guest_service_entities.md`).
