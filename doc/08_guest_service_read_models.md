...
* **Bill Summary** — order lines with prices and running total, for the Waiter to relay to the guest (`02_discover_process_level.md` §1.2). Fed by `OrderPlaced` (captures each order's lines + price) and `Bill`'s own `runningTotal`. Worth calling out explicitly: `Bill` itself doesn't store order lines (`08_guest_service_entities.md`) — this read model is what actually holds the itemised view; `Bill` only holds the running total needed for its own guards.
