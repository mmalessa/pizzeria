...
> **Note on this repository's current stage:** `pizzeria` is presently documentation-only — a DDD modelling exercise under `doc/` and `adr/`, with no code yet (see `AGENTS.md` for the layout). The process below still applies to documentation changes (a doc edit is a PR like any other), but the validation gate is empty until the Symfony implementation described in `doc/09_architecture.md` begins, and the QA gate is off since there is no UI to exercise yet. Revisit both — and this note — once implementation starts.
...
Note: `qaGate` is currently **off** in `.ai/agentic.config.json` (there is no UI yet to QA). `needs-qa`/`qa-approved`/`qa-failed` remain part of the label taxonomy for when UI work starts, but until `qaGate` is turned on, `needs-qa` is advisory only and does not block a merge.
...
_No validation commands are configured yet — this repository has no code to typecheck, test, or build (see the note at the top of this document). Add commands to `validation.commands` in `.ai/agentic.config.json`, and list them here, once implementation begins._
